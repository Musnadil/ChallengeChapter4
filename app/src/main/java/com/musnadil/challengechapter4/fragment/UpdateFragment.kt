    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnUpdateBarang.setOnClickListener {
            ...
            GlobalScope.async {
                ...
                runBlocking {
                    if (result != 0){
                        Toast.makeText(it.context, "Harga ${itemSelected.item_name} sukses diubah", Toast.LENGTH_SHORT).show()
                    }else{
                        Toast.makeText(it.context, "Harga gagal diubah", Toast.LENGTH_SHORT).show()
                    }
                    activity?.finish()
                }
            }
